...
def analytics_summary_compat(db: Session = Depends(get_db)) -> AnalyticsSummary:
    return analytics_summary(db)


@router.get("/journey")
def journey_overview_compat(db: Session = Depends(get_db)):
    return journey_overview(db)
